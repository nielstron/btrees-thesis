...
\begin{lemma}
    If all subtrees are of order k except for one, which has order$^a$ k,
...
The balancedness invariance follows again by first showing height invariance
...
For example for split\_max, rather than
requiring balancedness and $\order k$ for $k > 0$
on the full tree,
it is only required that the node contains two subtrees
at the end that have equal height and that the last tree has the same, recursive property.
Using this property was beneficial inside the proof,
...
Nevertheless, generating the most general lemmas usually proved
very useful, for example \autoref{lem:nodei-order}, yielding the reusability of $\node_i$ for
...
\begin{lemma}
    If the t is nonempty with to equally high last trees,
    and t is not a leaf $\Longrightarrow$ \\
