...
In the following, $Leaf$ is an empty tree and hence represents
...
Using the Isabelle Refinement Framework % TODO how to refer to this
...
data-structure.
A list $a$ is represented by a partially filled array,
if the array represents a list, of which the first $length a$
elements are equal to $a$.
Formally,
...
definition is_pfa where c l $\equiv$ $\lambda$(a,n). $\exists_A$ l'. a $\mapsto_a$ l' *  $\uparrow$(c = length l' $\wedge$ n $\le$ c $\wedge$ l = (take n l'))"
\end{lstlisting}
...
that copies a slice of an array to the slice of another array.
...
to lower indices or to higher indices.
...
Most triples are straightforward to derive which is why they are not
...
the operations are destructive, or in-place, or not.
For most operations, the approach that required the least
memory allocations was chosen.
...
The only thing that changed is the content of $di$.\footnote{
    In the actual proof, we even show that the array itself has not changed
    (i.e. has not been reallocated) if this is possible.
    The notation does however not yield high readability and is hence approximated
    here.
}
The $take$ function was refined differently,
as the cheapest and in our context consistent option is to simply reduce the number of
elements that are considered part of the represented list.
