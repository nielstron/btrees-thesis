...
The precise inner working of the split function are not of interest here
...
function are useful and 2) only relying on an abstract specification of
...
It scans linearly through the list, returning the first tupel that could potentially contain
the value $x$}
\label{fig:linear_split}

\end{figure}

Therefore all set functions are defined based on an abstract
split function that fulfils the following requirements:

\begin{itemize}
    \item $\splitfun xs\ p = (ls,rs) \Longrightarrow ls @ rs = xs$
