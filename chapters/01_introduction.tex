...
\begin{lstlisting}[mathescape=true, language=Isabelle]
$\langle$ is_pfa c tsi a * list_assn (btree_assn k $\times_a$ id_assn) ts tsi * true $\rangle$
imp_split a
$\langle\lambda$ i. is_pfa c tsi a * list_assn (btree_assn k $\times_a$ id_assn) ts tsi
  * $\uparrow$(split_relation ts (split ts p) i)$\rangle_t$
\end{lstlisting}
...
\begin{lemma}
    \begin{center}
        $\langle$is\_pfa c xs (a,n) * true $\rangle$ \\
        lin\_split (a,n) p \\
        $\langle \lambda$i.
        is\_pfa c xs (a,n) 
        * $\uparrow$(i $\leq$ n 
