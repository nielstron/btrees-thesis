...
Balancedness requires as additional lemma that
the height is invariant under insertion, which follows
...
trees, however we have shown that this is even the case for non-balanced trees
as this fact is slightly more general.

Note that for the above proofs, a separate notion of height, balancedness and order
was introduced for the $up_i$ datatype, which is exactly
the original notion applied to the B-Tree obtained by applying the $tree_i$ function.
Exceptions are the notion of order and height, which are expressed only with respect to
the subtrees.
The reason is to make these properties invariant with respect to applying the $node_i$
function to list and last tree,
which will simplify proofs later using this abstraction.
